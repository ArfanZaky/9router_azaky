...
export function shouldResolveProjectId(credentials) {
    const projectId = credentials?.projectId;
    if (typeof projectId === "string") return projectId.trim().length === 0;
    if (projectId && typeof projectId === "object") return !String(projectId.id || "").trim();
    return true;
}
