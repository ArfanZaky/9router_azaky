...
async function fetchJob(jobId) {
  ...
  return { res, data: await readJsonResponse(res, "Failed to fetch Blackbox job") };
}
...
async function fetchLatestJob() {
  ...
  return { res, data: await readJsonResponse(res, "Failed to fetch latest Blackbox job") };
}

export default function BlackboxAutomationModal({ isOpen, onClose, onSuccess }) {
  ...
  const startJob = async () => {
    ...
    try {
      ...
      const data = await readJsonResponse(res, "Blackbox bulk automation failed");
      if (!res.ok || data.error) throw new Error(data.error || "Blackbox bulk automation failed");
      setJob(data.job);
  ...
  return (
    <Modal isOpen={isOpen} title="Blackbox AI Bulk Signup + API Key" onClose={onClose} size="lg">
        ...
        {!job && (
          <>
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-200">
              Creates a disposable mailbox via catchmail.io, signs up a Blackbox.ai account with a generated email:password, verifies the email OTP, creates an API key from <code className="rounded bg-blue-100 px-1 dark:bg-blue-800">app.blackbox.ai/keys</code>, and saves it as a Blackbox provider connection.
            </div>
            ...
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
              <p className="font-medium">Heads up — API access approval</p>
              <p className="mt-1">
                Blackbox sometimes requires accounts to be approved for API access before keys can be created. If a signup succeeds but key creation returns <code className="rounded bg-amber-100 px-1 dark:bg-amber-800">Failed to create key</code>, the account was registered but is not yet eligible. Retrying later or from a different IP/proxy may help; some disposable-mail domains are treated more strictly.
              </p>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-1 text-xs text-text-muted">
                  One account = one temp email + one signup + one API key. Concurrency defaults to min(count, 4).
                </p>
              </div>
              <div>
            ...
            <div className="rounded-lg border border-border bg-background/70 p-3 text-xs text-text-muted">
              ...
              <ul className="mt-1 list-disc space-y-1 pl-4">
                <li>Each account gets a fresh generated <code className="rounded bg-background px-1">email@tempdomain</code> and a random 16-char password.</li>
                <li>The browser signs up at <code className="rounded bg-background px-1">app.blackbox.ai/signup</code>, polls catchmail.io for the 6-digit OTP, verifies, then creates the API key.</li>
                <li>The harvested <code className="rounded bg-background px-1">sk-...</code> key is saved as a Blackbox provider connection and is immediately usable for model calls.</li>
              </ul>
            </div>
        ...
        {job && (
          <div className="space-y-4">
              ...
              <div className="relative bg-black/90">
                {job.preview?.imageData ? (
                  <Image
                    ...
                    alt={`Live Blackbox worker preview for ${job.preview.email || "signup"}`}
                    width={1440}
                    height={900}
                    unoptimized
                  ...
                  <div className="flex h-[320px] flex-col items-center justify-center gap-3 px-6 text-center text-slate-200">
                    ...
                    <div>
                      <p className="text-base font-medium">Preview will appear when a worker opens Blackbox</p>
            ...
            {manualAccounts.length > 0 && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-200">
                Manual assist is needed. Open the browser worker, finish the CAPTCHA prompt, and the job will continue.
        ...
        {!job && (
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={startJob} disabled={startDisabled}>
              {starting ? "Starting..." : `Sign Up ${requestedCount} Blackbox Account${requestedCount === 1 ? "" : "s"}`}
            </Button>
          </div>
        )}
      </div>
...
BlackboxAutomationModal.propTypes = {
