...
import { Card, Button, Badge, Input, Modal, CardSkeleton, OAuthModal, KiroOAuthWrapper, CursorAuthModal, XiaomiMimoAuthModal, IFlowCookieModal, GitLabAuthModal, Toggle, Select, EditConnectionModal, NoAuthProxyCard, ConfirmModal, SegmentedControl, CodeBuddyQuotaCookieModal } from "@/shared/components";
...
export default function ProviderDetailPage() {
  ...
  const handleBulkUpdateConnectionStatus = async (ids, isActive) => {
    if (!ids.length) return;
    setConfirmState({
      title: isActive ? "Activate Connections" : "Inactive Connections",
      message: `${isActive ? "Activate" : "Inactive"} ${ids.length} selected connection(s)?`,
      onConfirm: async () => {
        setConfirmState(null);
        try {
          const results = await Promise.all(ids.map((id) => fetch(`/api/providers/${id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ isActive }),
          })));
          if (results.some((res) => !res.ok)) alert("Some connections failed to update.");
          await fetchConnections();
          clearSelection();
        } catch (error) {
          console.log("Error bulk updating connections:", error);
        }
      }
    });
  };

  const handleBulkDeleteConnections = async (ids) => {
    if (!ids.length) return;
    setConfirmState({
      title: "Delete Connections",
      message: `Delete ${ids.length} selected connection(s)?`,
      onConfirm: async () => {
        setConfirmState(null);
        try {
          const results = await Promise.all(ids.map((id) => fetch(`/api/providers/${id}`, { method: "DELETE" })));
          if (results.some((res) => !res.ok)) alert("Some connections failed to delete.");
          await fetchConnections();
          clearSelection();
        } catch (error) {
                  ...
                  {selectedConnectionIds.length > 0 && (
                    <>
                      <Button
                        ...
                        variant="secondary"
                        icon="block"
                        onClick={() => handleBulkUpdateConnectionStatus(selectedConnectionIds, false)}
                      >
                        Inactive ({selectedConnectionIds.length})
                      </Button>
                      <Button
                        size="sm"
                        variant="danger"
