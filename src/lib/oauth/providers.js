...
const PROVIDERS = {
  ...
  freebuff: {
    ...
    flowType: "device_code",
    // Freebuff uses a custom device flow: POST /api/auth/cli/code → auth_code,
    // browser Google SSO at codebuff.com/login?auth_code=XXX, then poll
    // /api/auth/cli/status until authToken is ready.
    requestDeviceCode: async (config) => {
