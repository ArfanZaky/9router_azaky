...
function sanitizeRequest(body = {}) {
  return {
    ...
    params: body.params || {},
    maxSteps: Math.min(Math.max(Number(body.maxSteps) || 12, 1), 30),
  };
}
...
async function runPlainChat(run) {
  ...
  if (preferJson || contentType.includes("application/json") || !response.body) {
    ...
    if (body.usage) run.tokenUsage = body.usage;
...
async function execute(run) {
  ...
  try {
    ...
    const content = stoppedSummary(run.messages, run.assistantText, stopped);
