...
describe("Antigravity bulk import OAuth URL", () => {
  it("uses the dashboard callback URL instead of a random loopback port", () => {
    expect(buildLoopbackRedirectUri()).toBe("http://localhost:2026/callback");
  });

  it("builds the same Google OAuth shape as the dashboard add-connection flow", () => {
    const authUrl = new URL(createAntigravityAuthUrl("http://localhost:2026/callback", "state-123"));
    ...
    expect(authUrl.searchParams.get("redirect_uri")).toBe("http://localhost:2026/callback");
  ...
  it("requests the dashboard authorize endpoint used by add connection", async () => {
    ...
    const data = await buildDashboardAntigravityAuthData("http://localhost:2026/callback");

    expect(data.state).toBe("from-dashboard");
    expect(fetch).toHaveBeenCalledWith(
      new URL("http://localhost:2026/api/oauth/antigravity/authorize?redirect_uri=http%3A%2F%2Flocalhost%3A2026%2Fcallback"),
      { cache: "no-store" },
    );
  });
  ...
  it("saves OAuth tokens even when loadCodeAssist returns no project", async () => {
    ...
    const result = await exchangeAndSaveAntigravityConnection({
      ...
      redirectUri: "http://localhost:2026/callback",
