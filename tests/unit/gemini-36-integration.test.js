...
describe("Gemini Cloud Code endpoint isolation", () => {
  it("reuses a persisted project instead of rediscovering it per chat turn", () => {
    expect(shouldResolveProjectId({ projectId: "existing-project" })).toBe(false);
    expect(shouldResolveProjectId({ projectId: { id: "existing-project" } })).toBe(false);
    expect(shouldResolveProjectId({ projectId: "" })).toBe(true);
    expect(shouldResolveProjectId({})).toBe(true);
  });
  it("keeps Gemini CLI on the official cloudcode host", async () => {
    ...
    expect(geminiCli.transport.baseUrl).toBe("https://daily-cloudcode-pa.googleapis.com/v1internal");
